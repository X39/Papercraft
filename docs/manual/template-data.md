...
This follows the same Boolean condition behavior verified by `IfTransformerTests.IfConditionUsesBooleanVariable`.
...
This pattern is verified by `SwitchTransformerTests.SwitchMatchesStringCase` and
`SwitchTransformerTests.SwitchUsesDefaultWhenNoCaseMatches`.
...
This list shape is verified by `ForEachTransformerTests.ForEachLoopWithVariableSource`; the generated
