...
`TemplateLanguageDocumentationSamples.TemplateLanguage_SwitchStatus`, and the temporary-value task sample is generated
by `TemplateLanguageDocumentationSamples.TemplateLanguage_TemporaryValue`. Optional-value `@if` patterns are checked by
