...
These fragments use area attributes verified against `Template.CreateAsync` and `AreaSample`:
...
This fragment uses the left/right width behavior verified against `Template.Area.CalculateClippingAndTranslationData`:
