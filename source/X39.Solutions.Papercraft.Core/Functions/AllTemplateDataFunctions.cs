...
namespace X39.Solutions.Papercraft.Functions;
...
internal class AllTemplateDataFunctions : IFunction
{
        ...
        CancellationToken cancellationToken = default)
    {
        return ValueTask.FromResult<object?>(_templateData.Functions
            .Select(
                (function) =>
                    $"{function.Name}({string.Join(", ", Enumerable.Range(0, function.Arguments).Select((argNo) => $"arg{argNo}").Append(IsVariadic ? "..." : null).OfType<string>())})"));
    }
}
