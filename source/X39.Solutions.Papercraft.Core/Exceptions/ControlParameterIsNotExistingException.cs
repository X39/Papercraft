...
public sealed class ControlParameterIsNotExistingException : Exception
{
    ...
    internal ControlParameterIsNotExistingException(
        ...
        : base($"The parameters {string.Join(", ", missingParameters)} do not exist in control {controlType.FullName ?? controlType.Name}.{Environment.NewLine}Available parameters: {string.Join(", ", availableParameters)}")
    {
