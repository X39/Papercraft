...
public readonly record struct Color(byte Red, byte Green, byte Blue, byte Alpha = 255) : ISpanParsable<Color>
{
    ...
    // ReSharper disable once InconsistentNaming
    public static Color FromHSL(ushort hue, float saturation, float light, byte alpha = 255)
    {
        var l = light;
        var s = saturation;
        var h = hue;
    ...
    public static bool TryParse(ReadOnlySpan<char> s, IFormatProvider? provider, out Color result)
    {
        ...
        switch (s.Length)
        {
            case 3:
            {
                ...
                var g = s[1];
                var b = s[2];
                Span<char> tmp = stackalloc char[] { r, r, g, g, b, b, 'F', 'F' };
                if (uint.TryParse(tmp, NumberStyles.HexNumber, provider, out var value))
                {
            ...
            case 4:
            {
                ...
                var b = s[2];
                var a = s[3];
                Span<char> tmp = stackalloc char[] { r, r, g, g, b, b, a, a };
                if (uint.TryParse(tmp, NumberStyles.HexNumber, provider, out var value))
                {
                    result = new Color(value);
                    return true;
            ...
            case 6:
            {
                Span<char> tmp = stackalloc char[] { s[0], s[1], s[2], s[3], s[4], s[5], 'F', 'F' };
                if (uint.TryParse(tmp, NumberStyles.HexNumber, provider, out var value))
                {
                    result = new Color(value);
                    return true;
